...
import CustomerOptionLabel from "@/components/CustomerOptionLabel";
...
const CustomerSetupFormContent = ({ isLoading, form, isUpdate }: IField) => {
  ...
  const debouncedFetchCustomers = useMemo(
    () => debounceAsync(loadCustomerOptions, 500),
    [loadCustomerOptions],
  );
  ...
  const customerSetupStatusOptionsFromEnum = useGenerateDropdownOptionsFromEnum(
    CUSTOMER_SETUP_STATUS_ENUM,
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <AsyncDropDownComponent<ICustomer>
            ...
            formatOptionLabel={(val) => (
              <CustomerOptionLabel customer={val.value} />
            )}
            disabled={isLoading}
            options={debouncedFetchCustomers}
            width="100%"
          />
          ...
          <Controller
            ...
            name="scheduledStart"
            rules={{
              required: !isUpdate ? "Scheduled Start is required" : false,
            }}
            render={({ field }) => (
          ...
          <Controller
            ...
            render={({ field }) => (
              <div className="space-y-1 w-full">
                <p className="text-xs text-onCard font-medium">Scheduled End</p>
                <DatePicker
                  title=""
