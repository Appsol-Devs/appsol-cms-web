...
const CustomerOutreachForm = () => {
      ...
      .then((res) => {
        if (res?.contents) {
          setOutreachTypeOptions(
            res.contents.map((type: { name?: string; id?: string }) => ({
              label: type.name ?? "",
              value: type.id ?? "",
            })),
          );
      ...
      .then((res) => {
        if (res?.contents) {
          setCallStatusOptions(
            res.contents.map((status: { name?: string; id?: string }) => ({
              label: status.name ?? "",
              value: status.id ?? "",
            })),
          );
  ...
  const values = watch();
  const [selectedData, setSelectedData] = useState<ICustomerOutreach | null>(
    null,
  );
  ...
  const submitData = () => {
    ...
    const requiredFields: { field: unknown; message: string }[] = [
      ...
      {
        field: data.outreachType?.value,
        message: "Outreach Type is required.",
      },
      {
        field: data.callStatus?.value,
        message: "Call Outcome/Status is required.",
      },
  ...
  const summarySections: ISummarySection[] = [
    {
      ...
      data: [
        {
          label: "Customer",
          value: values?.customer?.label as string,
          required: true,
        },
    ...
    },
    {
      ...
      data: [
        {
          label: "Notes",
          value: values?.notes ? "Provided" : "Empty",
          required: false,
        },
      ],
    },
    ...
    <div>
      <MutationFormTemplate<ICustomerOutreachFields>
        ...
        pageSummary={{
          ...
          description: `Record details of your interaction with the customer${
            id ? " (Editing)" : ""
          }.`,
          icon: PhoneOutgoing,
        }}
