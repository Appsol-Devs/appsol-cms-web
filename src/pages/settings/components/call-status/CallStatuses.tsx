...
import { useLazyGetCallStatusesQuery } from "../../common/settingsApi";
import ActionButton from "@/components/ActionButtons";

const CallStatuses = () => {
  const [fetchQuery, fetchState] = useLazyGetCallStatusesQuery();
  ...
  const columns = useMemo<ColumnDef<ICallStatus>[]>(
    () => [
      {
        header: "#",
        accessorKey: "index",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className=" flex flex-col items-start gap-1">
            ...
            <Badge>
              {row.original?.callStatusCode ? row.original.callStatusCode : ""}
            </Badge>
          </div>
        ),
      },
          ...
          <div className="flex items-center space-x-2">
            <ActionButton
              ...
              onClick={() =>
                navigate(
                  allRoutes.PORTAL +
                    allRoutes.UPDATE_CALL_STATUS(row.original._id as string)
                )
              }
            />
          </div>
      ...
      <FeatureContentRenderer
        tableAddComponent={() => (
          <ActionButton
            type="add"
            onClick={() =>
              navigate(allRoutes.PORTAL + allRoutes.ADD_CALL_STATUS)
            }
          />
        )}
        columns={columns}
