...
import { useLazyGetLeadNextStepsQuery } from "../../common/settingsApi";
import ActionButton from "@/components/ActionButtons";

const LeadNextSteps = () => {
  const [fetchQuery, fetchState] = useLazyGetLeadNextStepsQuery();
  ...
  const columns = useMemo<ColumnDef<ILeadNextStep>[]>(
    () => [
      {
        header: "#",
        accessorKey: "index",
      ...
      <FeatureContentRenderer
        tableAddComponent={() => (
          <ActionButton
            type="add"
            onClick={() =>
              navigate(allRoutes.PORTAL + allRoutes.ADD_LEAD_NEXT_STEP)
            }
          />
        )}
        columns={columns}
