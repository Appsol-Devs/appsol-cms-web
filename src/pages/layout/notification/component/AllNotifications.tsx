...
import { useLazyGetPaginatedNotificationsQuery, useMarkAsReadMutation } from "../common/notificationsApi";
    ...
    const columns = useMemo<ColumnDef<INotification>[]>(
        () => [
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    const hasComplaintLink = row.original.link && row.original.link.includes("complaints");
                            ...
                            {hasComplaintLink ? (
                                <span
                                    ...
                                    onClick={(e: React.MouseEvent<HTMLSpanElement>) => {
                                        e.stopPropagation();
                                        const id = row.original.link?.split("/").filter(Boolean).pop();
                                        if (id) {
                    ...
                    <div className="flex items-center space-x-2">
                        {!row.original.isRead ? (
                            <ActionButton
                                ...
                                useText="Mark Read"
                                onClick={() => {
                                    handleMarkAsRead(String(row.original._id));
                                }}
                            />
                        ) : (
