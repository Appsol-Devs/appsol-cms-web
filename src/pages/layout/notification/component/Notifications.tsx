...
import { Bell, X, FileText, Check, List } from "lucide-react";
  ...
  return (
    <>
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <PopoverContent
          align="end"
          className="w-[300px] p-0 shadow-lg rounded-sm overflow-hidden text-xs"
        >
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col gap-1 text-onSurface">
                {modifiedNotifications.map((notif) => {
                  const initials = `${notif.user?.firstName?.[0] || ""}${
                    notif.user?.lastName?.[0] || ""
                  }`;
